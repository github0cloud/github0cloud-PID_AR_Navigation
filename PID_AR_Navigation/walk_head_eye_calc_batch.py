    ...
    for X,Xname in enumerate(Xname_list):
        ...
        for angle_direction in angle_directions:
            for vis_name in vis_names:
                ...
                # random_list = np.random.rand(len(SI_List))
                # t_statistic, p_value = stats.ttest_ind(SI_List, random_list)
                # print(f"SI_List vs random_list: t_statistic = {t_statistic}, p_value = {p_value}")


                # random_list = np.random.rand(len(UIY_List))
                # t_statistic, p_value = stats.ttest_ind(UIY_List, random_list)
                # print(f"UIY_List vs random_list: t_statistic = {t_statistic}, p_value = {p_value}")


                # random_list = np.random.rand(len(UIZ_List))
                # t_statistic, p_value = stats.ttest_ind(UIZ_List, random_list)
                # print(f"UIZ_List vs random_list: t_statistic = {t_statistic}, p_value = {p_value}")

                # random_list = np.random.rand(len(CI_List))
                # t_statistic, p_value = stats.ttest_ind(CI_List, random_list)
                # print(f"CI_List vs random_list: t_statistic = {t_statistic}, p_value = {p_value}")
                ...
                # ######################### A2 test #########################
                # df = pd.read_csv('SA_result.csv', usecols=[0])

                # avg_aclrtReci_list = df['avg_aclrtReci_list'].tolist()



                # def result_show(name,test_List, avg_aclrtReci_list):
                
                #   print("------",name,"------:")
                #   test_List = scalers.minMax_Scaler(test_List)
                #   avg_aclrtReci_list = scalers.minMax_Scaler(avg_aclrtReci_list)

                #   model = LinearRegression()
                #   model.fit(np.array(test_List).reshape(-1, 1), np.array(avg_aclrtReci_list))

                #   slope = model.coef_[0]
                #   intercept = model.intercept_

                #   predictions = model.predict(np.array(test_List).reshape(-1, 1))

                #   corla_cofi.print_CC_pv(test_List,avg_aclrtReci_list)
                #   plt.scatter(test_List, avg_aclrtReci_list, color='blue', label='Data Points')
                #   plt.plot(test_List, predictions, color='red', linewidth=2, label='Linear Regression')
                #   plt.legend()
                #   plt.show()

                #   print("slope:", slope)

                # result_show("SI",SI_List, avg_aclrtReci_list)
                # result_show("UIY",UIY_List, avg_aclrtReci_list)
                # result_show("UIZ",UIZ_List, avg_aclrtReci_list)
                # result_show("CI",CI_List, avg_aclrtReci_list)



# fwrite = open("used_time.txt", "w")
# fwrite.write(str(end_time - start_time))
# fwrite.close()